...
set "PROCESS_EXE=%BTC5M_BACKUP_EXE_PATH%"
if not defined PROCESS_EXE set "PROCESS_EXE=%LOCALAPPDATA%\Python\pythoncore-3.14-64\btc5m-backup-dataset.exe"
if not exist "%PROCESS_EXE%" powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%ENSURE_SCRIPT%" -Quiet >nul 2>nul
if not exist "%PROCESS_EXE%" set "PROCESS_EXE=%LOCALAPPDATA%\Programs\Python\Python314\btc5m-backup-dataset.exe"
if not exist "%PROCESS_EXE%" set "PROCESS_EXE=%LOCALAPPDATA%\Programs\Python\Python311\btc5m-backup-dataset.exe"
...
"%PROCESS_EXE%" "scripts\btc5m_backup_dataset.py"
exit /b %errorlevel%
